...
void sendToMonitoring(char* msg, char* argv[]) {
    ...
    servAddr.sin_port = htons(atoi(argv[20]));
...
void sendResponse(char* msg, char* argv[]) {
    ...
    char* info = "[SERVER LAWYER] Send response %s to child with id=%s";
...
int main(int argc, char* argv[]) {
    ...
    while (true) {
        ...
        char* info = "[SERVER LAWYER] Got request from child with id=%s";
